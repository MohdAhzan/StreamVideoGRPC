...
import (
	...
	interfaces "github.com/MohdAhzan/StreamVideoGRPC/VIDEO_STREAM_SVC/pkg/repository/interface"

	"github.com/google/uuid"
)

const storageLocation = "videos"
...
func (c *VideoServer) StreamVideo(req *pb.StreamVideoRequest, stream pb.VideoService_StreamVideoServer) error {
  ...
  fmt.Println("REACHED STREAM VIDEO SERVICES IN VIDEOSVC")
	chunkSize := 4096 // Set your desired chunk size
	buffer := make([]byte, chunkSize)
	playlistPath := fmt.Sprintf("videos/%s/%s", req.Videoid, req.Playlist)
	playlstFile, _ := os.Open(playlistPath)
	defer playlstFile.Close()
	for {
		n, err := playlstFile.Read(buffer)
		if err == io.EOF {
			break
		}
		if err != nil {
